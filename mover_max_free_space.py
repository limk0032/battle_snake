...
class MoverMaxFreeSpace(object):
    ...
    def get_preferred_directions_to_food(
        ...
        if food["x"] == my_head["x"] and food["y"] < my_head["y"]:
            preferred_direction = self.combine_preferred_directions_with_possible_moves(
    ...
    def re_prioritize_preferred_directions_based_on_free_connected_tile(
        ...
        for move in possible_moves_prioritized:
            possible_move_index = self.possible_move_to_index(move, my_head)
            number_of_connected_tile = self.get_number_of_connected_tile(
    ...
    def get_number_of_connected_tile(
        ...
        number_of_connected_tile = self.get_number_of_connected_tile_recursively(
    ...
    def get_number_of_connected_tile_recursively(
        ...
        result = 1
        result = result + self.get_number_of_connected_tile_recursively(
    ...
    def get_move_with_max_free_space(self, data):
        ...
        possible_moves = self.avoid_my_neck(my_head, my_body, possible_moves)
        possible_moves = self.avoid_border(
            my_head,
            my_body,
            possible_moves,
            data["board"]["height"],
        ...
        if data["board"]["snakes"]:
            for snake in data["board"]["snakes"]:
                possible_moves = self.avoid_my_neck(my_head, snake["body"], possible_moves)
        # print(f'possible_moves:{possible_moves}')
        if data["board"]["food"]:
            nearest_food = self.get_foods_sorted_by_distance_asc(
                my_head, data["board"]["food"]
            )[0]
            print(f"nearest_food:{nearest_food}")
            possible_moves_prioritized = self.get_preferred_directions_to_food(
            ...
            possible_moves_prioritized = (
                self.re_prioritize_preferred_directions_based_on_free_connected_tile(
                    my_head,
                    my_body,
                    data["board"]["snakes"],
                    data["board"]["height"],
