...
def populate_min_step_to_reach_matrix(min_step_to_reach_matrix: List[List[int]], body: List[dict], other_snakes_body: List[dict], board_height: int, board_width: int):
    ...
    while matrix_before_change != matrix_after_change:
        ...
        for x in range(board_width):
            for y in range(board_height):
                ...
                surrounding_tiles = list(filter(lambda n: min_step_to_reach_matrix[n['x']][n['y']] != '-', surrounding_tiles))
                if len(surrounding_tiles) == 0:
                    continue
                surrounding_min_steps = list(map(lambda t: min_step_to_reach_matrix[t['x']][t['y']], surrounding_tiles))
                surrounding_min_steps = list(dict.fromkeys(surrounding_min_steps))  # unique min steps
                surrounding_min_steps = sorted(surrounding_min_steps, key=lambda n: n)
                for surrounding_min_step in surrounding_min_steps:
                    surrounding_tiles_with_min_steps = list(filter(lambda n: min_step_to_reach_matrix[n['x']][n['y']] == surrounding_min_step, surrounding_tiles))
                    if len(surrounding_tiles_with_min_steps) == 0:
                        continue
                    if surrounding_min_step == 0:
                        not_accessible_tiles = other_snakes_body + body
                    else:
                        not_accessible_tiles = other_snakes_body + body[:-(surrounding_min_step)]
                    ...
                    if origina_value == '-':
                        ...
                        min_step_to_reach_matrix[x][y] = min(surrounding_min_step + 1, min_step_to_reach_matrix[x][y])
...
def populate_shortest_paths_tree(min_step_to_reach_matrix: List[List[int]], start, end, board_height, board_width, parent_tree_node):
    if start == end:
        return
    # debug
    surrounding_tiles = get_surrounding_tiles(start, board_height, board_width)
    surrounding_tiles = list(filter(lambda t: min_step_to_reach_matrix[t['x']][t['y']] != '-', surrounding_tiles))
    if len(surrounding_tiles) == 0:
        return
    ...
    surrounding_tiles_with_min_steps = list((filter(lambda t: min_step_to_reach_matrix[t['x']][t['y']] == min_step_to_reach_matrix[start['x']][start['y']] - 1, surrounding_tiles)))
    if len(surrounding_tiles_with_min_steps) > 0:
        for sub_tile in surrounding_tiles_with_min_steps:
            ...
            populate_shortest_paths_tree(min_step_to_reach_matrix, sub_tile, end, board_height, board_width, sub_tree_node)
...
def get_shortest_paths_to_target(body, other_snakes, board_height, board_width, target):
    ...
    populate_min_step_to_reach_matrix(min_step_to_reach_matrix, body, other_snakes_body, board_height, board_width)
    # print_min_step_to_reach_matrix(min_step_to_reach_matrix)
    #
    ...
    populate_shortest_paths_tree(min_step_to_reach_matrix, target, {'x': body[0]['x'], 'y': body[0]['y']}, board_height, board_width, shortest_paths_tree_root_node)
...
def get_first_shortest_path_to_food(data: dict):
    ...
    for food in foods:
        min_step_to_reach_matrix_head_to_food, shortest_paths_head_to_food = get_shortest_paths_to_target(body, other_snakes, board_height, board_width, food)
        ...
        for path in shortest_paths_head_to_food:
            ...
            min_step_to_reach_matrix_food_to_tail, shortest_paths_food_to_tail = get_shortest_paths_to_target(body_after_eating_food, other_snakes, board_height, board_width, tail_after_eating_food)
...
def get_move_in_shortest_path_to_food(data):
    ...
    if len(first_shortest_path_to_food) != 0:
        ...
        print('chase tail')
        snakes_without_my_body = list(filter(lambda s: s['body'] != data["you"]["body"], data["board"]["snakes"]))
        my_body_without_tail = data["you"]["body"]
        min_step_to_reach_matrix, paths_to_tail = get_shortest_paths_to_target(my_body_without_tail, snakes_without_my_body, data['board']['height'], data['board']['width'], data["you"]["body"][-1])
        print_min_step_to_reach_matrix(min_step_to_reach_matrix)
...
data = {'game': {'id': '63d80eb8-22f4-4ea3-9b6a-29c9b7cc61b3', 'ruleset': {'name': 'solo', 'version': 'v1.0.22', 'settings': {'foodSpawnChance': 15, 'minimumFood': 1, 'hazardDamagePerTurn': 0, 'royale': {'shrinkEveryNTurns': 0}, 'squad': {'allowBodyCollisions': False, 'sharedElimination': False, 'sharedHealth': False, 'sharedLength': False}}}, 'timeout': 500, 'source': ''}, 'turn': 169,
        'board': {'height': 7, 'width': 7, 'snakes': [{'id': 'gs_R3FTmRbx6HQMvMBfr3KWJ6w7', 'name': 'snake1', 'latency': '301', 'health': 100, 'body': [{'x': 0, 'y': 5}, {'x': 0, 'y': 4}, {'x': 0, 'y': 3}, {'x': 0, 'y': 2}, {'x': 1, 'y': 2}, {'x': 1, 'y': 1}, {'x': 1, 'y': 0}, {'x': 2, 'y': 0}, {'x': 2, 'y': 1}, {'x': 2, 'y': 2}, {'x': 3, 'y': 2}, {'x': 3, 'y': 1}, {'x': 4, 'y': 1}, {'x': 5, 'y': 1}, {'x': 6, 'y': 1}, {'x': 6, 'y': 2}, {'x': 6, 'y': 3}, {'x': 5, 'y': 3}, {'x': 5, 'y': 2}, {'x': 4, 'y': 2}, {'x': 4, 'y': 3}, {'x': 4, 'y': 4}, {'x': 3, 'y': 4}, {'x': 3, 'y': 5}, {'x': 4, 'y': 5}, {'x': 5, 'y': 5}, {'x': 5, 'y': 4}, {'x': 6, 'y': 4}, {'x': 6, 'y': 5}, {'x': 6, 'y': 6}, {'x': 5, 'y': 6}, {'x': 4, 'y': 6}, {'x': 3, 'y': 6}, {'x': 2, 'y': 6}, {'x': 1, 'y': 6}, {'x': 1, 'y': 6}], 'head': {'x': 0, 'y': 5}, 'length': 36, 'shout': '', 'squad': ''}], 'food': [{'x': 3, 'y': 3}, {'x': 6, 'y': 0}], 'hazards': []},
        'you': {'id': 'gs_R3FTmRbx6HQMvMBfr3KWJ6w7', 'name': 'snake1', 'latency': '301', 'health': 100, 'body': [{'x': 0, 'y': 5}, {'x': 0, 'y': 4}, {'x': 0, 'y': 3}, {'x': 0, 'y': 2}, {'x': 1, 'y': 2}, {'x': 1, 'y': 1}, {'x': 1, 'y': 0}, {'x': 2, 'y': 0}, {'x': 2, 'y': 1}, {'x': 2, 'y': 2}, {'x': 3, 'y': 2}, {'x': 3, 'y': 1}, {'x': 4, 'y': 1}, {'x': 5, 'y': 1}, {'x': 6, 'y': 1}, {'x': 6, 'y': 2}, {'x': 6, 'y': 3}, {'x': 5, 'y': 3}, {'x': 5, 'y': 2}, {'x': 4, 'y': 2}, {'x': 4, 'y': 3}, {'x': 4, 'y': 4}, {'x': 3, 'y': 4}, {'x': 3, 'y': 5}, {'x': 4, 'y': 5}, {'x': 5, 'y': 5}, {'x': 5, 'y': 4}, {'x': 6, 'y': 4}, {'x': 6, 'y': 5}, {'x': 6, 'y': 6}, {'x': 5, 'y': 6}, {'x': 4, 'y': 6}, {'x': 3, 'y': 6}, {'x': 2, 'y': 6}, {'x': 1, 'y': 6}, {'x': 1, 'y': 6}], 'head': {'x': 0, 'y': 5}, 'length': 36, 'shout': '', 'squad': ''}}

